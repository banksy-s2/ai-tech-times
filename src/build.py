...
function pwiki(name, bio){
  ...
  // 同名の別項目(例:「トランプ」→カードゲーム)を避けるため紹介文も検索語に添える
  var q = key + " " + (bio || "").slice(0, 24);
  fetch("https://ja.wikipedia.org/w/api.php?action=query&list=search&format=json&origin=*&srlimit=1&srsearch=" + encodeURIComponent(q))
