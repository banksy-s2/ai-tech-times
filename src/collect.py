...
# 記事の新規生成を停止したカテゴリ(2026-07-25 緊急役員会D1・社長裁可)
# voices: 個人の評論・エッセイを日本語に翻訳する行為は著作権法27条(翻訳権)に触れる恐れがある。
# 対象3氏(Willison/Mollick/Clark)のブログにCC等の許諾表示はなく(C)表記のみであることを確認済み。
# 既存ページ・ナビ・SOURCES定義は残す(リンク+1行紹介への作り替えは実装凍結明けに着手)。
# 解除するときは必ず鵜飼(セキュリティ)と社長の承認を取ること。
PAUSED_CATEGORIES = {"voices"}
# 停止が発効した日時(JST)。これ以降に公開された停止カテゴリの記事は「漏れ」として検知する。
# 同日の午前中まで既存記事があるため、日付だけでは既存と漏れを区別できない(Codex第8回指摘4)
PAUSED_SINCE = "2026-07-25 20:00"
