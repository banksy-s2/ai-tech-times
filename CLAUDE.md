...
- フル便(7/12/17/21時): 全7カテゴリ12本+X告知
- 軽量便(他の毎時): 1カテゴリ順繰りで1〜2本(`run_pipeline.py` の `MAIN_HOURS` と hour%7 ローテ)
- カテゴリ第7弾「株式投資(日本)」は2026-07-22追加(stock。NHK経済/Googleニュース/東洋経済/はてブ経済。投資助言NG・事実のみの執筆ルール付き)
...
9. **`data/articles.json` に空の`[]`を置かない**。台帳が読めないのか本当に空なのかを区別できないため、
   `build._check_ledger()` が「ファイルはあるのに記事ゼロ」を異常とみなして便を止める。
   ゼロから始めるならファイルごと削除する
10. **毎時便がcommitするのは `data docs company` のみ**。`src/` や `run_pipeline.py` を変更したら
   **手動でcommitすること**(放置すると`git pull --rebase --autostash`で毎時退避・復元され続ける)

## 著作権(2026-07-25 緊急役員会。最優先の制約)

**当社の最大のリスクはGoogleではなく著作権と利用規約**。調査結果は `company/research/2026-07-25-legal-risk.md`。

- **`collect.PAUSED_CATEGORIES`** = 記事の新規生成を止めたカテゴリ。現在 `{"voices"}`(海外AI識者の
  個人ブログの日本語翻訳)。**翻訳は翻訳権(27条)に直撃し、対象3氏のブログにCC等の許諾表示はない**
  - 三重で止めている: ①便のカテゴリ選定から除外 ②`build.save_articles`が保存を拒否
    ③`precheck._paused_leak`が漏れを毎便検知(重大警報→便を中止)
  - **解除は鵜飼と社長の承認が必要**。`PAUSED_SINCE`(停止発効日時)も併せて更新する
- 新しいソースを足す前に **robots.txtとRSS利用規約**を読む(Yahoo!ニュースRSSは公開サイトでの利用を明示禁止)
- **他社の記事本文をLLMに渡して要約させるのは翻案の疑いが濃い**(日本新聞協会の公式見解)。
  中期の方針は「事実だけを抽出 → 元テキストを渡さず別工程で執筆」への移行(緊急役員会D2)
...
## 実装フロー(2026-07-25 社長指示・**例外なし**)

**コードを実装したら、必ずCodexレビューを通してから完了報告する。**

```
着工前チェック(鵜飼) → 実装 → 自己検証 → **Codexレビュー** → 指摘対応 → 台帳記録 → 社長へ報告
```

- 実行: `mcp__codex__codex` を `sandbox: read-only`, `approval-policy: never`, `cwd: リポジトリ` で呼び、
  **前回レビュー以降の差分**を重点に、重要度順・ファイル:行番号付き・日本語で依頼する
- レビュー観点は必ず含める: セキュリティ / 正確性 / **破壊的操作の誤動作** / 運用への影響 / リグレッション
- **重大(高)の指摘は、その場で修正してから報告する**。後回しにしない
- 結果は `company/security/RISK_LEDGER.md` の監査履歴に追記する(件数・重大の有無・対応)
- 対象外: 文書のみの変更(会議録・規程)、記事データの手直し。**コードとテンプレートの変更は必ず対象**

**なぜ**: 作者は自分の思い込みの外側が見えない。第6回で「自作の掃除機能が619ファイルを消しうる」欠陥を、
外部の目が公開前に発見した。レビューを挟まない実装は、事故を公開してから気づくことになる。
